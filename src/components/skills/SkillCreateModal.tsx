...
export default function SkillCreateModal({ isOpen, onClose, onSuccess }: SkillCreateModalProps) {
  ...
  const validate = (): boolean => {
    ...
    if (!formData.chapter.trim()) {
      newErrors.chapter = 'Chương là bắt buộc';
    } else if (formData.chapter.length > 255) {
      newErrors.chapter = 'Chương không được vượt quá 255 ký tự';
    }
      ...
      <div className="p-4 sm:p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            />
            {errors.chapter && <p className="mt-1 text-sm text-red-500">{errors.chapter}</p>}
          </div>
