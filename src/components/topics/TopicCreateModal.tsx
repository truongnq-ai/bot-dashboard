...
import { createTopic } from '@/lib/api/topic.service';
...
export default function TopicCreateModal({ isOpen, onClose, onSuccess }: TopicCreateModalProps) {
  ...
  const { data: topics } = useTopics({ subjectId: formData.subjectId || undefined });

  // Filter topics by selected subject for parent dropdown
  const availableParentTopics = useMemo(() => {
    if (!formData.subjectId) return [];
    return topics || [];
  }, [topics, formData.subjectId]);
  ...
  const validate = (): boolean => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = 'Tên topic là bắt buộc';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await createTopic(requestData);
      if (response.errorCode === '0000') {
        showSuccess('Tạo topic thành công');
        onSuccess?.();
        onClose();
      } else {
        showError(response.errorDetail || 'Tạo topic thất bại');
      }
    } catch (error) {
          ...
          <div>
            ...
            <select
              className={`w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white ${
                errors.subjectId ? 'border-error-500' : 'border-gray-300 dark:border-gray-600'
              }`}
              value={formData.subjectId}
              onChange={(e) => {
                setFormData({ ...formData, subjectId: e.target.value, parentId: undefined });
              }}
            >
            ...
            {errors.subjectId && <p className="mt-1 text-theme-sm text-red-500">{errors.subjectId}</p>}
          </div>
